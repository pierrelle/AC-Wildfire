...
__colors__ = [(255, 255, 255), (6, 176, 0), (230, 107, 0)]
...
class Grid:
    ...
    def __init__(self):
        ...
        self._grid = np.random.randint(2, size=(__gridDim__), dtype="int8") > 0.99
    ...
    def sumEnumerate(self):
        return [
            (c, self.sommeVoisins(c[0], c[1])) for c, _ in np.ndenumerate(self._grid)
        ]
...
class Scene:
    ...
    def update(self):
        ...
        for c, s in self._grid.sumEnumerate():
            self._grid._gridbis[c[0], c[1]] = (
                1 if (2 <= s <= 4) and self._grid._grid[c[0], c[1]] == 0 else 0
            )
